...
def build_tokenizer(args):
    ...
    elif args.tokenizer_type == 'PretrainedTokenizerFromHF':
        assert args.tokenizer_model is not None
        tokenizer = _PretrainedTokenizerFromHF(args.tokenizer_model)
    else:
...
class _PretrainedTokenizerFromHF(_SentencePieceTokenizer):
    """SentencePieceTokenizer-Megatron wrapper"""

    def __init__(self, model_file,):
        name = 'HF Pretrained Tokenizer'
        super().__init__(name)
    ...
    def tokenize(self, s: str, bos=True, eos=False):
        ...
        t = self.tokenizer.encode(s)
        if bos:
            t = [self.tokenizer.bos_token_id] + t
        if eos:
