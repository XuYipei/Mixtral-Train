...
class RotaryEmbedding(nn.Module):
    def __init__(self, kv_channels: int, rotary_percent: float, rotary_interleaved: bool = False, seq_len_interpolation_factor: float = None, rotary_base=1000000.0):
        super().__init__()
    ...
    def _set_cos_sin_cache(self, seq_len):
        ...
        self.register_buffer("cos_cached", emb.cos().to(dtype=self.inv_freq.dtype), persistent=False)
        self.register_buffer("sin_cached", emb.sin().to(dtype=self.inv_freq.dtype), persistent=False)

    def forward(self, max_seq_len, offset=0):
        # x: [bs, num_attention_heads, seq_len, head_size]
        if max_seq_len + offset > self.max_seq_len_cached:
            self._set_cos_sin_cache(seq_len=max_seq_len + offset)

        return torch.concat(
            [
                self.cos_cached[offset: max_seq_len + offset].unsqueeze(0),
                self.sin_cached[offset: max_seq_len + offset].unsqueeze(0)
            ],
            dim = 0
        )
...
def apply_rotary_pos_emb(t, freqs, config: TransformerConfig, cu_seqlens: Optional[Tensor] = None,):
    cos = freqs[0]
    sin = freqs[1]
    from einops import rearrange
    t = rearrange(t, 'seq bs nh dim -> bs nh seq dim')
    t_embed = (t * cos) + (rotate_half(t) * sin)
    t_embed = rearrange(t_embed, 'bs nh seq dim -> seq bs nh dim')
    return t_embed