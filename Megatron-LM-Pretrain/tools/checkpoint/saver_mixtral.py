...
def save_checkpoint(queue, args):
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer in range(len(models[0].language_model.encoder.layers)):
            ...
            mlp_router_weight = msg.pop("mlp router weight")
            ...
            mlp_w1_weight_W_per_expert = torch.chunk(msg.pop("mlp experts weight1 W"), margs.num_experts, dim=-1)
            mlp_w1_weight_V_per_expert = torch.chunk(msg.pop("mlp experts weight1 V"), margs.num_experts, dim=-1)
            mlp_w2_weight_per_expert = torch.chunk(msg.pop("mlp experts weight2"), margs.num_experts, dim=0)
            
            mlp_w1_weight_W_per_expert_tp = [torch.chunk(w, args.target_tensor_parallel_size, dim=-1) for w in mlp_w1_weight_W_per_expert]
            mlp_w1_weight_V_per_expert_tp = [torch.chunk(v, args.target_tensor_parallel_size, dim=-1) for v in mlp_w1_weight_V_per_expert]
            mlp_w2_weight_per_expert_tp = [torch.chunk(w, args.target_tensor_parallel_size, dim=0) for w in mlp_w2_weight_per_expert]

            for tp_rank in range(args.target_tensor_parallel_size):
                layer_chunk = models[tp_rank].language_model.encoder.layers[layer]
                w1_weight = layer_chunk.mlp.experts.weight1.data.copy_
                w2_weight = layer_chunk.mlp.experts.weight2.data.copy_


                w1_weight_md = torch.stack(
                    [
                        torch.cat([w[tp_rank], v[tp_rank]], dim=-1) for (w, v) in zip(mlp_w1_weight_W_per_expert_tp, mlp_w1_weight_V_per_expert_tp)
                    ],
                    dim=0
                ).view(margs.hidden_size, -1)
                w1_weight(w1_weight_md)
                
                w2_weight(
                    torch.stack(
                        [
                            w[tp_rank] for w in mlp_w2_weight_per_expert_tp
                        ],
                        dim=0
                    ).view(-1, margs.hidden_size)
                )
            ...
            for tp_rank in range(args.target_tensor_parallel_size):
                ...
                layer_chunk.mlp.router.weight.data.copy_(mlp_router_weight)
